...
bad_pages = ['https://www.acu.edu.au/about-acu/executive-education/our-courses/executive-postgraduate/master-of-family-and-systemic-therapy']

course_data = dict()
course_data_all = []
for each_url in course_links_file:
    for bad_url in bad_pages:
        if each_url == bad_url:
            continue
            ...
            if div_x:
                ...
                if ul:
                    ...
                    if li:
                        ...
                        print('INTERNATIONAL COURSE PRICE: ',
                              int_costs.strip().replace('\n', '').replace('<', '').replace('>', ''))
...
with open('ACU_postgraduate.csv', 'w', encoding='utf-8', newline='') as output_file:
    dict_writer = csv.DictWriter(output_file, course_dict_keys)
    dict_writer.writeheader()
    dict_writer.writerows(course_data_all)
